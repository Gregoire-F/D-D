document.addEventListener("DOMContentLoaded", async () => {
    ...
    function renderMonster(data) {
        ...
        <div class="monster-card">
            ...
            ${image ? `<div style="text-align:center;"><img src="${image}" style="max-width:300px; border-radius:8px; margin-bottom:20px;"></div>` : ""}
            ...
            <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:10px; text-align:center; background:#eee; padding:15px; border-radius:8px; margin-bottom:20px;">
                <div><strong>CA</strong><br>${data.armor_class?.[0]?.value || "-"}</div>
                <div><strong>PV</strong><br>${data.hit_points}</div>
                <div><strong>Vitesse</strong><br>${formatSpeed(data.speed)}</div>
    ...
    function renderClass(data) {
        ...
        <div class="monster-card">
            ...
            <div class="monster-content">
                ...
                <p><strong>Compétences :</strong> ${data.proficiencies.map(p => p.name).join(", ")}</p>
            </div>
        </div>`;
    }
    ...
    // Helpers
    function formatSpeed(s) { return s ? Object.entries(s).map(([k, v]) => `${k}: ${v}`).join(", ") : "-"; }
    function formatList(l) { if (!l) return ""; return `<ul>${l.map(i => `<li><strong>${i.name}:</strong> ${i.desc}</li>`).join("")}</ul>`; }
    function renderStats(d) {
        const stats = ["strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma"];
        return stats.map(s => {
            const mod = Math.floor((d[s] - 10) / 2);
            return `<div><small>${s.slice(0, 3).toUpperCase()}</small><br><strong>${d[s]}</strong><br><small>${mod >= 0 ? "+" + mod : mod}</small></div>`;
        }).join("");
    }
});